...
func _process(_delta: float) -> void:
	...
	var charge_ratio_pos: float = max(-0.05, charge_ratio - 0.05)
	charge_rect.color = charge_color_ramp.sample(charge_ratio)
	charge_rect.scale.x = (charge_ratio_pos) * max_width + 0.05
	
	charge_border.modulate = charge_color_ramp.sample(charge_ratio)
	charge_border.position.x = lerp(840, 1030, charge_ratio_pos) + 5
